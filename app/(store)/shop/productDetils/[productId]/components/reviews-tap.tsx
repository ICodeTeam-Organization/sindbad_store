...
const ProductReviewsTap: React.FC<ProductReviewsTapProps> = ({ productId }) => {
  ...
  const totalRating = reviews.reduce((acc: number, review: ReviewProps) => acc + review.numOfRate, 0);
  ...
  if (error) return <div>حدث خطأ أثناء جلب المراجعات.</div>;
    ...
    <div className="mdHalf:grid grid-cols-3 gap-6" dir="rtl">
      ...
      <div className="col-span-2">
        ...
        {reviews.length === 0 ? (
          ...
          <>
            {reviews.slice(0, visibleReviewsCount).map((review: ReviewProps, index: number) => (
              <ReviewComment
                key={index}
                reviewer={review.customerName}
                date={new Date(review.reviewDate).toLocaleDateString()}
