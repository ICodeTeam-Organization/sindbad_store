        ...
        <div className="block md:hidden" dir="rtl">
          {orders.length > 0 &&
            orders.map(({ name, specialCategoryId, note, createdAt, id, typeName }, index) => (
              <div
                key={index}
                className={`border rounded-lg p-4 mb-4 ${index % 2 !== 0 ? "bg-[#FFFBF8]" : "bg-white"
                  }`}
              >
                ...
                <div className="flex items-center justify-between">
                  <Link
                    href={"/my-special-orders/priceDetails/" + id}
                    className="w-full mt-2 text-center text-sm text-white shadow-sm h-[38px] bg-secondary px-4 py-2 rounded-lg whitespace-nowrap"
                  >
                    عرض تفاصيل الطلب
                  </Link>
                </div>
