...
const PreviousOrdersTable: React.FC<{ initData: SpecialOrdersResponseType }> = ({ initData }) => {
    ...
    const { data, isRefetching, fetchNextPage, hasNextPage, isFetchingNextPage } =
      useInfiniteQuery({
        queryKey: ["customerPrevsOrders"],
        queryFn: async ({ pageParam = 1 }) => {
          return getApi<SpecialOrdersResponseType>(
            `SpecialProducts/Market/GetSpecialProductsByCustomerFilter/300/10/${pageParam}`
          );
        },
    ...
    return (
     ...
     {orders.length <= 0 && (
       ...
       {orders.length > 0 && <table className="w-full border-collapse">
          {/* Table Header */}
          ...
          <tbody className="text-sm text-center text-[#000]">
            {orders?.map(
              (
                { name, specialCategoryId, note, createdAt },
                index
              ) => (
                <tr
                  key={index}
                ...
                >
                  ...
                  <td className="px-4 py-3">{convertToArabicDate(createdAt)}</td>
