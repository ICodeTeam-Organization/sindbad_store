...
export default function SpecialOrderMegaMenu() {
  ...
  return (
    <div className={cn(
      ...
      <div className={cn("flex flex-col xl:w-[60%] mdHalf:w-[50%] w-full mdHalf:h-auto h-[90%]  px-4 py-8  text-sm text-gray-500 dark:text-gray-400 md:grid-cols-3 md:px-6 gap-x-4",CatType != 'p' && "mdHalf:flex hidden")}>
         ...
         ? <div className='grid gap-x-4  mb-5 ' >
              {
                categories.filter((i)=>i.categoryTypeNumber==2)?.map((i,x)=>(
                    <Link key={i.id + x} href={"/special-order?sh=1&tab=1&category="+i.id}  className='text-[11px] me-2 font-semibold my-[2px] hover:bg-gray-200  transition-colors duration-200 px-2 p-1 rounded  text-black ' > {i.name} </Link>
                ))
              }
          </div>
      ...
      <div className={cn("flex flex-col xl:w-[60%] mdHalf:w-[50%] w-full mdHalf:h-auto h-[90%]  px-4 py-8  text-sm text-gray-500 dark:text-gray-400 md:grid-cols-3 md:px-6 gap-x-4",CatType != 'k' && "mdHalf:flex hidden")}>
         ...
         ? <div className='grid  gap-x-4  mb-5 ' >
              {
                  categories.filter((i)=>i.categoryTypeNumber==3)?.map((i,x)=>(
                      <Link key={i.id+x} href={"/special-order?sh=1&tab=2&category="+i.id}  className='text-[11px] me-2 font-semibold my-[2px] hover:bg-gray-200  transition-colors duration-200 px-2 p-1 rounded  text-black ' > {i.name} </Link>
                  ))
              }
          </div>
        : <div className='p-5 flex items-center justify-center ' >
