...
function EShopsMegaMenu() {
 ...
 const [selectedCategory, setselectedCategory] = useState(categories[0].id)
  ...
  return (
  //   <div className="transition-all duration-200 opacity-0 invisible hidden  mdHalf:block  group-hover:block  -translate-y-5  group-hover:-translate-y-0 w-full group-hover:opacity-100 group-hover:visible mdHalf:mt-1 -mt-2 rounded top-10 left-0   max-h-[476px] mdHalf:overflow-y-hidden overflow-y-scroll z-[99999]  bg-white  mdHalf:shadow-md mdHalf:border-y border-b dark:bg-gray-800  mdHalf:absolute   ">
  ...
  //     <div className="flex mdHalf:block  bg-white flex-col lg:w-[50%] mdHalf:w-[40%] justify-between mdHalf:h-[440px] overflow-hidden ">
  //       <div className="hidden mdHalf:flex items-center  gap-2 mb-4  ">
  //         <BiCategoryAlt size={25} color="black" />
  //         <h3 className="text-lg font-bold text-black  "> إختر فئة </h3>
  //       </div>
  //       <div className="mdHalf:grid lg:grid-cols-2 flex  mdHalf:place-content-start  mdHalf:overflow-y-scroll mdHalf:overflow-x-hidden overflow-x-scroll  gap-x-4  mdHalf:mb-5 mb-2  mdHalf:h-[75%]">
  //         {categories.slice(0, 10).map((i) => (
  //           <p
  //             onClick={() => setselectedCategory(i.category)}
  //             className={cn(
  //               " text-[14px] lg:text-sm my-[2px] hover:bg-gray-200 text-black font-semibold transition-colors duration-200 h-fit px-2 p-1 mdHalf:rounded rounded-full xl:whitespace-nowrap mdHalf:whitespace-normal whitespace-nowrap ",
  //               selectedCategory == i.category && "bg-slate-200"
  //             )}
  //           >
  //             {" "}
  //             {i.category}{" "}
  ...
  //       <Link
  //         href="/"
  //         className="hover:underline hover:text-blue-600 mdHalf:mb-0 mb-4 "
  //       >
  //         <p>عرض الكل</p>
  //       </Link>
  //     </div>
  //     {/* <div className='h-[180px] w-[1px] mx-4 bg-[#AAA7A7] rounded-full my-auto' /> */}
  //     <div className="mdHalf:border-r  border-[#AAA7A744]  mdHalf:pr-4  mdHalf:h-[475px] w-full ">
  //       <div className=" items-center  gap-2 mb-4 hidden mdHalf:flex">
  //       <LucideShoppingBag size={25} color="black" />
  //         <h3 className="text-lg font-bold text-black "> المتاجر الإلكترونية  </h3>
  //       </div>
  //       <div className="mdHalf:grid xl:grid-cols-5 lg:grid-cols-4 mdHalf:grid-cols-3 grid-cols-1 place-content-start  gap-x-4   overflow-y-scrol overflow-x-hidden h-[75%] ">
  //         {categories
  //           .find((o) => o.category == selectedCategory)
  //           ?.stores.map((i) => (
  //             <p className=" text-[14px] my-[2px] h-fit hover:bg-gray-200 font-semibold transition-colors duration-200 px-2 p-1 rounded lg:whitespace-nowrap">
  //               {" "}
  //               {i}{" "}
  //             </p>
  //           ))}
