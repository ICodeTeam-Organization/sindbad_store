      ...
      {isLoading ? (
            ...
            <div className="mdHalf:grid xl:grid-cols-7 lg:grid-cols-6 mdHalf:grid-cols-4 grid-cols-1 place-content-start  gap-x-4   overflow-y-scrol overflow-x-hidden h-[75%] ">
              {data?.data?.items?.map((i) => (
                <Link href={i?.urlLinkOfStore} target='_blank' key={i.id} className=" text-[11px] my-[2px] h-fit hover:bg-gray-200 font-semibold transition-colors duration-200 px-2 p-1 rounded lg:whitespace-nowrap">
                  {" "}
                  {i.name}{" "}
                </Link>
              ))}
