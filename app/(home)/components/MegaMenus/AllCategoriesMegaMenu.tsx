      ...
      <div className="flex mdHalf:flex-row flex-col px-4 py-5 mx-auto text-sm text-gray-500 dark:text-gray-400 md:px-6 gap-x-4 w-full">
        ...
        <div className="mdHalf:border-r  border-[#AAA7A744]  mdHalf:pr-4  mdHalf:max-h-[400px] xl:w-[50%] mdHalf:w-[50%] ">
          ...
          {subCategories.length != 0 ? (
            <div className="mdHalf:grid grid-cols-1 place-content-start  gap-x-4   overflow-y-scrol  overflow-x-hidden h-[75%]" dir="ltr" >
              {[...subCategories]?.map((i: MainCategory) => (
                <Link key={i.id} href={`/shop?subCat=${i.id}`} className=" block text-[11px] my-[2px] text-end me-2 h-fit hover:bg-gray-200 font-semibold transition-colors duration-200 px-2 p-1 rounded lg:whitespace-nowrap">
                  {" "}
                  {i.name}{" "}
                </Link>
              ))}
