          ...
          {isLoading ? (
            ...
            <div className="mdHalf:grid  xl:grid-cols-5 2lg:grid-cols-4 mdHalf:grid-cols-3 grid-cols-1 place-content-start  gap-x-4   overflow-y-scrol overflow-x-hidden h-[75%] ">
              {data?.data?.items?.map((i) => (
                <Link href={"/stores/storeDetails/"+i.id} key={i.id} className=" text-[11px] my-[2px] h-fit hover:bg-gray-200 font-semibold transition-colors duration-200 px-2 p-1 rounded lg:whitespace-nowrap">
                <div className="flex gap-3 items-center " >
                  <div className="rounded-lg overflow-hidden w-12 border h-12 relative"  > 
                    <SafeImage
                      alt={i.name}
                      fill
