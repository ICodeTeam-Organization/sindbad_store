...
export default function GetFavorite() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const [productsResponse, storesResponse, ecommrcesResponse] = await Promise.allSettled([
        ...
        axios.get(
          process.env.NEXT_PUBLIC_BASE_URL +
            `FavoriteShop/GetFavoriteEcommerceStores?pageNumber=1&pageSize=2000`,
          {
            headers: {
              Authorization: `Bearer ${authData?.user.data.token}`,
            },
  ...
  useEffect(() => {
    ...
    if (data) {
      ...
      setFavoriteEcommerceIds(Array.isArray(data.ecommrces?.items) ? data.ecommrces.items.map((e) => e.ecommerceStoreId) : []);
      setFavoriteStoreIds(Array.isArray(data.stores?.items) ? data.stores.items.map((e) => e.storeId) : []);
    }
  }, [data, authData]);
