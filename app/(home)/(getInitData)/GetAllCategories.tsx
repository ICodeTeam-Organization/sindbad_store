...
export default function GetAllCategories({date}:{date:Date}) {
  ...
  const lastupdateOfCategories = typeof window !== "undefined" ?  localStorage.getItem("CATS_LAST_UPDATE_"+ getCookie("country")) : null;
...
useEffect(() => { 
  if (isSuccess && data) {
    const updateLocalDatabase = async () => {
      const localCats = await db.categories.toArray(); 
      const localCatKeys = new Set(localCats.map(cat => JSON.stringify([cat.country, cat.id]))); 
      const newCategories = [];
      const updatedCategories = []; 
      for (const cat of data) {
      ...
      if (newCategories.length > 0 || updatedCategories.length > 0) {
        const now = date || new Date();
        now.setHours(now.getHours() + 2);
        localStorage.setItem("CATS_LAST_UPDATE_" + getCookie("country"), now.toISOString());
      } 
      const finalCats = await db.categories.toArray();
      setCategories(finalCats);
    };
