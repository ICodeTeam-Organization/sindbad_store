...
const PrintOrderBill = ({ Bill }: { Bill: OrderData }) => {
  ...
  const totalDiscount = calculateTotalDiscount(orderItems.map(e=>({
                priceAfterDiscount: null,
                price: e.price,
                quantity: e.quantity,
              })));

  const finalPrice = calculateFinalTotal( orderItems.map((e) => ({
                shipCost: e.shipCost,
                quantity: e.quantity,
                amountYouBuy: 0,
                amountYouGet: 0,
    ...
    <div>
      ...
      <Card className="mdHalf:sticky mdHalf:top-[100px] mdHalf:z-10 border-white shadow-none  mdHalf:border-gray-200 ">
        ...
        <CardContent className="text-sm p-0 mdHalf:p-6 mdHalf:pt-0 pb-6 ">
          ...
          <div className="flex justify-between mb-2">
            <span className="font-semibold">الإجمالي</span>
            <span className="font-semibold">{finalPrice} رس</span>
          </div>
        </CardContent>
        {
          <CardFooter>
