...
function FavoriteProducts() {
  ...
  const {
    ...
    queryFn: async ({ pageParam = 1 }) => {
      const localData = await db.bgData.where("reqType").equals(1).toArray();
  ...
  return (
    <div dir="rtl" className="mb-12 flex flex-wrap justify-center mdHalf:gap-6 gap-3">
      {isLoading ? (
        ...
        <>
          {allProducts.map((product: NormalizedProductType) => (
            <div key={product.id} className="sm:w-[220px] w-[180px]">
          ...
          {hasNextPage && (
            <div className="w-full flex justify-center mt-6">
              <button
                ...
                className="bg-primary text-white px-8 py-3 rounded-md text-sm   font-bold"
              >
        ...
        <div className="h-[65vh] flex items-center justify-center w-full">
          <p className="text-center text-lg tajawal font-bold py-12">
            لايتوفر أي منتج في المفضلة
          </p>
        </div>
